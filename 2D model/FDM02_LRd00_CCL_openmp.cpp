...
void performance();
void comp_stim_time();//Calculating new stimulation time
...
/* set the stimulus flag and time */
void comp_stim_time(){
	if (t >= tstim && t<(tstim + dt_max)){
		stimtime = 0;
		//b_i = b_i + 1;
		tstim = tstim + bcl;
		printf("t:%g\n", t);
...
void Rush_Larsen(int i, int j, double dt){
	...
	double tauzdv = 1 / (azdv + bzdv);
	double zssdv = azdv / (azdv + bzdv);
	zdv[i][j] = zssdv - (zssdv - zdv[i][j])*exp(-dt / tauzdv);
	double aydv = 0.015 / (1 + exp((v[i][j] + 60) / 5));
	double bydv = (0.1*exp((v[i][j] + 25) / 5)) / (1 + exp((v[i][j] + 25) / 5));
	double tauydv = 1 / (aydv + bydv);
	double yssdv = aydv / (aydv + bydv);
